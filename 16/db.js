...
function DB() {
  ...
  this.updateOneRecord = async (tableName, args) => {
    ...
    this.getTeachersByFaculty = async (args, context) => {
      ...
      let dataToReturn = [];
      rows.forEach((item) => {
        dataToReturn.push({FACULTY: item.FACULTY,TEACHERS: [{TEACHER: item.TEACHER,TEACHER_NAME: item.TEACHER_NAME,PULPIT: item.PULPIT,}]})
      });
      return dataToReturn;
    };
    ...
    this.getSubjectsByFaculties = async (args, context) => {
      ...
      let dataToReturn = [];
      rows.forEach((item) => {
        dataToReturn.push({FACULTY: item.FACULTY,PULPIT: item.PULPIT,PULPIT_NAME: item.PULPIT_NAME,
