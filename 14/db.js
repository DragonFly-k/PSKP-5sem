...
class DB {
    ...
    async getFaculties() {
        return this.connectionPool.then((conn) =>
            conn.request().query('Select * FROM FACULTY')
        )
    }

    async getPulpitsByFaculty(faculty) {
        return this.connectionPool.then((conn) =>
            conn.request().input('faculty', sql.VarChar, faculty)
    ...
    async getPulpits() {
        return this.connectionPool.then((conn) =>
            conn.request().query('Select * FROM PULPIT')
        )
    }

    async getSubjects() {
        return this.connectionPool.then((conn) =>
            conn.request().query('Select * FROM SUBJECT')
        )
    }

    async getAuditoriumTypes() {
        return this.connectionPool.then((conn) =>
            conn.request().query('Select * FROM AUDITORIUM_TYPE')
        )
    }

    async getAuditoriumType(type) {
        return this.connectionPool.then((conn) =>
            conn.request().input('type', sql.VarChar, type)
    ...
    async getAuditoriums() {
        return this.connectionPool.then((conn) =>
            conn.request().query('Select * FROM AUDITORIUM')
        )
    }
    ...
    async getPulpit(pulpit) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('pulp', sql.NVarChar, pulpit)
    ...
    async getFaculty(faculty) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('facul', sql.NVarChar, faculty)
                .query('Select * from FACULTY where faculty = @facul')
        })
    }

    async getSubject(subject) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('sub', sql.NVarChar, subject)
    ...
    async getAuditorium(audit) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('audit', sql.NVarChar, audit)
    ...
    async postAuditorium(auditorium,auditorium_name,auditorium_capacity,auditorium_type) {
        return this.connectionPool.then((conn) => {
            return conn.request()
    ...
    async putAuditorium(auditorium,auditorium_name,auditorium_capacity,auditorium_type) {
        return this.connectionPool.then((conn) => {
            return conn.request()
                .input('auditorium', sql.NVarChar, auditorium)
                .input('auditorium_name', sql.NVarChar, auditorium_name)
    ...
    async deleteFaculty(faculty) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('faculty', sql.NVarChar, faculty)
    ...
    async deletePulpit(pulpit) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('pulpit', sql.NVarChar, pulpit)
    ...
    async deleteSubject(subject) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('subject', sql.NVarChar, subject)
    ...
    async deleteAuditoriumType(auditorium_type) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('auditorium_type', sql.NVarChar, auditorium_type)
    ...
    async deleteAuditorium(auditorium) {
        return this.connectionPool.then((conn) => {
            return conn.request().input('auditorium', sql.NVarChar, auditorium)
